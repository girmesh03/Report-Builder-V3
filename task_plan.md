...
**Sub-phase 4 — Domain APIs (§30–§36) — IMPLEMENTED, SUITE GREEN — STEP 5 PENDING (2026-08-20).** Branch `phase-6-backend-domains` (pass 1 committed `c722298`, pushed). Full plan (D1–D26, edge-case matrix 20 rows, endpoint inventory, suite design) approved twice ("Proceed … apply the implementation phases protocol … exhaustively"). Scope pins: D8 additive ledger `stt.audios` on Transcription; D21 ledger excluded from the DTO; D22 row language = am (only activated code); rateLimit `isTieredPath` gains the PUT-transcription ai-tier rule + the method-aware chat POST rule (§33.8/§36.7). Files: utils/{transaction,pagination,sanitizer,ethiopianDate,wavSplitter}.js; services/{addis,gemini,nvidia}-provider.js, provider-chain.js, {stt,generation,correction,chat}.service.js; controllers/routes/validators ×5; scripts/test-04-domains.mjs; models: paginate plugin ×7 + stt ledger; env.js FFMPEG_PATH/FFPROBE_PATH + SDK instance; test-02 stt-surface amendment; httpStatus 206/416. **Suite `test-04-domains.mjs` — ALL GROUPS GREEN: unit 9, branches 17, reports 35, audio 11, transcription 7, realpipeline 22 (the owner-provided 187.7 s webm → 4 real addis STT chunks → real addis generation → §6 render with ± labels → items → gemini surgical correction (SC-3 token gate) → gemini chat), ratelimit 12 (429 at #11), sourcegates 10; regressions test-01 12/12, test-02 39/39, test-03 37/37.** Integration fixes in the same set: addis SDK response-surface mapping (`ChatCompletion` shape), Gemini schema normalization (union types → 400), item PATCH manual validation (no runValidators — OQ-021), root-mounted router auth per-route (404-envelope fix), stale MongoDB indexes dropped (unique branch index violated §20.3), merged ledger trim. Mirrors applied: §15.4 tree, §10.4 ffmpeg rows, §11.6 206/416, §27.3 ai-tier row, §16.4 SDK-surface fact, §23.2/§33.6 ledger, §38.7 C5 gate, §13.3/§13.5 addisai install, §69 D-series + OQ-020/021 + sub-phase-4 record. NEXT: step 5 = owner live run (restart per group) → step 6 gated commit.
