...
**Sub-phase 6 — Seeding & Sweepers (§40/§25/§62) — CLOSED (2026-08-21, step 6 executed on owner directive — commits `df99691` + `afc04e9`, merged to `main` at `39c10f8`, branch deleted; the step-5 owner live run was waived by owner directive).** Implemented: mock/fixtures.js (the canonical §25.3 set); mock/seed.js (self-replacing, one session, §21.8 refs, D45/D52 probe); mock/wipe.js (D41 signature + D44 guard); routes/mock.routes.js + the §40.5 conditional mount; jobs/sweeper.js (two passes, run guard, log-only) + server.js start/stop; scripts/test-06-seeding.mjs. **Suite green: test-06 34/34 (unit 5, seed 9, wipe 6, sweeper 5 — SW0–SW4, sourcegates 9).** F99 close-out audit fixes: dead sweeperTimer code removed, tallies corrected (34/sweeper 5), REAL_CLIP relocated to `backend/scripts/fixtures/` (+ §15.4 row), ጎላጉል fixture vestige removed. Integration fixes (F98): Report transcription index unique+sparse → **unique+partial $type objectId** (mirrors §17.3/§21.2/§21.3 + test-02), Atlas in-transaction parallel cursors sequentialized, Mongoose 9 `ordered: true`. Boot DB connection retry added (`afc04e9`, F100/D53: constants + connectWithRetry, §26.6/§11.3/§69 mirrors, verified live). **Stage 4 backend COMPLETE — the Stage-5 frontend hard gate is lifted.** Remaining: test-04 transcription+realpipeline pending the owner recording at `backend/scripts/fixtures/amharic-sample-recording.webm`; next work = frontend P4 remainder (§52 wizard + editor install), branched fresh from main.
