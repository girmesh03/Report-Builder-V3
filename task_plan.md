...
**Sub-phase 4 — Domain APIs (§30–§36) — PLAN APPROVED 2026-08-20, EXECUTION IN PROGRESS.** Full plan (D1–D26, edge-case matrix 20 rows, endpoint inventory, suite design) presented twice and approved ("Proceed … apply the implementation phases protocol … exhaustively"). Branch: `phase-6-backend-domains` (from `main` 875b13d). Scope pins: D8 resolved — additive ledger `stt.audios: [ObjectId]` on Transcription (§23.2/§33.6 mirror), within-call chunk skip, cross-call per-audio skip via the ledger, partial failures re-run all (row exists only pre-success), merge = single-space join asc createdAt/_id (D17), empty segments contribute nothing (D18); D21 ledger excluded from the DTO; D22 row language = first clip's. rateLimit real gap: `isTieredPath` gains the PUT-transcription ai-tier rule (§33.8). Files: utils/{transaction,pagination,sanitizer,ethiopianDate,wavSplitter}.js; services/{addis,gemini,nvidia}-provider.js, provider-chain.js, {stt,generation,correction,chat}.service.js; controllers/routes/validators ×5 (branch, report, audio, transcription, chat); scripts/test-04-domains.mjs; models: paginate plugin ×7 + stt ledger; env.js FFMPEG_PATH/FFPROBE_PATH + SDK instance; test-02 stt-surface amendment. Mirrors same-commit: §15.4 tree, §10.4 ffmpeg rows, §27.3 ai-tier row, §23.2/§33.6 ledger, §38.7 C5 gate amendment, §13.3/§13.5 install record, §69 (D9/D10/D13/D19/D22–D26). Gates: node --check all; test-04 green; one transition-guard table; uploads gitignored; AI keys only in backend/.env; step 5 = owner live run (restart per group) → step 6 gated commit.
