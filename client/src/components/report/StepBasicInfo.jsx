...
import { Controller, useController, useFormState, useWatch } from "react-hook-form";
 ...
 * @param {boolean} [props.branchesLoading] - True while branches fetch.
 * @param {Function} props.branchNameOf - Branch _id → name resolver.
 */
const StepBasicInfo = forwardRef(function StepBasicInfo(
  { control, branches, branchesLoading = false, branchNameOf },
  ref,
) {
  ...
  const syncMainVisit = (key, value) => {
    ...
    visitsField.onChange(
      visits.map((visit, index) => (index === 0 ? { ...visit, [key]: value } : visit)),
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        <FieldBlock label={WIZARD.fieldLabels.date} error={errors.date} ref={dateRef}>
          <Controller
            name="date"
            control={control}
            rules={{ validate: validateDate }}
            render={({ field }) => (
              <MuiDatePicker value={field.value} onChange={field.onChange} error={Boolean(errors.date)} />
            )}
          />
        </FieldBlock>
        <FieldBlock
      ...
      <Box ref={visitsRef} tabIndex={-1} sx={{ outline: "none", mt: 1 }}>
        <Typography variant="subtitle2" sx={{ mb: 0.5 }}>
